// SPDX-License-Identifier: MIT
pragma solidity ^0.8.28;

contract MultiManagedAccess {
    uint256 constant MANAGER_NUMBERS = 5;
    uint256 immutable BACKUP_MANAGER_NUMBERS;
    
    address public owner;
    address[MANAGER_NUMBERS] public manager;
    bool[MANAGER_NUMBERS] public confirmed;
    mapping(uint256 => bool[MANAGER_NUMBERS]) public comfirmedRq;

    constructor(address _owner, address[] memory _managers, uint _manager_numbers) {
        require(_managers.length == _manager_numbers, "size Unmatched");
        owner = _owner;
        BACKUP_MANAGER_NUMBERS = _manager_numbers;
        for(uint256 i = 0; i < _manager_numbers; i++) {
            manager[i] = _managers[i];
        }
    }

    modifier onlyOwner() {
        require(msg.sender == owner, "You are not authorized");
        _;
    }

    function allConfirmed() internal view returns (bool) {
        for(uint256 i = 0; i < MANAGER_NUMBERS; i++) {
            if(!confirmed[i]) {
                return false;
            }
        }
        return true;
    }       
    function reset() internal {
        for(uint256 i = 0; i < MANAGER_NUMBERS; i++) {
            confirmed[i] = false;
        }
    }

    modifier onlyAllConfirmed() {
        require(allConfirmed(), "Not all managers confirmed yet");
        reset();
        _;
    }

    function confirm() external {
        bool found = false;
        for(uint256 i = 0; i < MANAGER_NUMBERS; i++) {
            if(manager[i] == msg.sender) {
                found = true;
                confirmed[i] = true;
                break;
            }
        }

        require(found, "You are not one of managers");
    }
}